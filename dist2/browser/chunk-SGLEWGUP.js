...
}`)}function D(t){t.fragment.uniforms.add(new y("texWaveNormal",e=>e.waveNormal),new y("texWavePerturbation",e=>e.wavePerturbation),new m("waveParams",e=>F(ae,e.waveStrength,e.waveTextureRepeat,e.flowStrength,e.flowOffset)),new k("waveDirection",e=>M(te,e.waveDirection[0]*e.waveVelocity,e.waveDirection[1]*e.waveVelocity))),t.fragment.include(J),t.fragment.code.add(a`const vec2  FLOW_JUMP = vec2(6.0/25.0, 5.0/24.0);
...
}`)}var ae=_(),te=I();function K(t){t.fragment.uniforms.add(new i("cloudAbsorption",e=>e.clouds.absorption),new i("cloudCoverage",e=>e.clouds.coverage)).code.add(a`vec4 lookupCloudsFromTextureArray(sampler2DArray cubeMap, vec3 rayDir) {
int faceIndex;
vec2 uv;
if(rayDir.z <= 0.0) {
